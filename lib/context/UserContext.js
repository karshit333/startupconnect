...
export function UserProvider({ children }) {
  ...
  const markMessagesAsRead = useCallback(async (conversationId) => {
    ...
    // Calculate new count BEFORE updating - subtract messages from this conversation
    const previousUnread = unreadMessages
    
    // Count messages in this conversation that would be marked as read
    try {
      const { data: msgsToMark } = await supabase
        .from('messages')
        .select('id')
      ...
      const countToMark = msgsToMark?.length || 0
      const newCount = Math.max(0, previousUnread - countToMark)
      
      // OPTIMISTIC: Immediately update UI
      setUnreadCountOptimistic(newCount)
      ...
      if (error) {
        ...
        // Rollback on error
        setUnreadCountOptimistic(previousUnread)
        cache.unreadCount = previousUnread
      }
    } catch (error) {
      console.error('Error in markMessagesAsRead:', error)
    }
  }, [user?.id, supabase, unreadMessages, setUnreadCountOptimistic])
