...
export function UserProvider({ children }) {
  ...
  const [unreadNotifications, setUnreadNotifications] = useState(cache.notifCount)
  ...
  const refreshNotificationCount = useCallback(async (userId) => {
    if (!userId) return
    try {
  ...
  const loadUserData = useCallback(async (forceRefresh = false) => {
    ...
    if (!forceRefresh && cache.user && (now - cache.lastFetch) < 5000) {
      ...
      setUnreadNotifications(cache.notifCount)
      setIsLoading(false)
    ...
    try {
      ...
      const [profileResult, startupResult] = await Promise.all([
        ...
        supabase.from('startups').select('*').eq('user_id', authUser.id).maybeSingle()
      ])
      ...
      // Set startup if exists (regardless of profile role)
      if (startupData) {
      ...
      await Promise.all([
        refreshUnreadCounts(authUser.id, true),
        refreshNotificationCount(authUser.id)
      ])
  ...
  }, [supabase, refreshUnreadCounts, refreshNotificationCount])
  ...
  // Mark messages as read - COMPLETELY OPTIMISTIC with DB sync
  const markMessagesAsRead = useCallback(async (conversationId) => {
    if (!user?.id) return

    // Increment operation counter immediately
    markOperationsRef.current++
    
    // Add to marked conversations set to prevent real-time subscription from incrementing
    cache.markedConversations.add(conversationId)
    ...
    try {
      ...
      // IMMEDIATELY update the UI - don't wait for DB
      const newTotal = Math.max(0, cache.unreadCount - countInConvo)
      setUnreadImmediate(newTotal)
      ...
      // Keep the conversation in marked set for a while
      setTimeout(() => {
        cache.markedConversations.delete(conversationId)
      }, 5000) // 5 second grace period

    } catch (error) {
      console.error('Error in markMessagesAsRead:', error)
      // On error, refresh from DB to get correct count
      await refreshUnreadCounts(user.id, true)
    } finally {
      // Decrement operation counter
      markOperationsRef.current--
    }
  }, [user?.id, supabase, setUnreadImmediate, refreshUnreadCounts])

  // Decrement unread count directly (for use by messages page)
  const decrementUnread = useCallback((amount = 1) => {
    const newCount = Math.max(0, cache.unreadCount - amount)
    setUnreadImmediate(newCount)
  }, [setUnreadImmediate])
