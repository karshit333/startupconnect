...
export function UserProvider({ children }) {
  ...
  // Flag to prevent real-time subscription from overriding optimistic updates
  ...
  // Direct state update for unread count
  ...
  // Fetch unread message counts from database
  const refreshUnreadCounts = useCallback(async (userId, force = false) => {
    // Skip if we're in the middle of marking messages as read (unless forced)
    if (!force && isMarkingAsReadRef.current) {
      return
    }
  ...
  // Real-time subscription for NEW messages only (not updates)
  useEffect(() => {
    if (!user?.id) return
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        event: 'INSERT', // Only listen to INSERT, not UPDATE
        schema: 'public',
        table: 'messages'
      }, (payload) => {
        // Only increment count if this message is for us and not from us
        if (payload.new && payload.new.sender_id !== user.id) {
          // Check if this conversation involves the current user
          // For simplicity, just refresh the count
